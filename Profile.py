...
# OF THIS CODE RIGHT NOW, though can you certainly take a look at it
# if you are curious since we already covered a bit of the JSON
# format in class.
import json
import time
...
own code. It is raised when attempting to load or save
...
class Profile:
    ...
    add_post accepts a Post object as parameter and appends it to
    the posts list.
    Posts are stored in a list object in the order they are added.
    So if multiple Posts objects are created, but added to
    the Profile in a different order, it is possible for the
    ...
    del_post removes a Post at a given index and returns True if
    ...
    def load_profile(self, path: str) -> None:
        ...
        if p.exists() and p.suffix == '.dsu':
            ...
            print('Error finding file in load_profile')
            raise DsuFileError()
